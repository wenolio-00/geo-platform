...
MVP 版本默认 `candidate_queries = 40`。生成链路不再按模板列表前 N 条截断，而是先用矩阵格 allocation weight 计算每个模块数量，再从对应模板池抽取 query。

运营端如需调整总量，只允许通过运行约束传入具体数值：
...
后端使用 largest-remainder rounding：

```text
raw_score(cell) = 用户真实发生频率 × AI 推荐触发概率 × 商业决策价值 × 趋势可比性
allocation_weight(cell) = raw_score(cell) / sum(raw_score)
raw_count(cell) = total_candidate_queries × allocation_weight(cell)
final_count = floor(raw_count) + 按小数余数从大到小补齐剩余条数
```

其中 allocation weight 是生成数量来源；`metric_weight` 是主指标计算来源。两者不要混用。
...
## 10. MVP 默认 40 条 Query 的生成分配

当前 MVP 默认 40 条 query。条数保证业务覆盖，并作为 generation allocation 的默认权重来源；主指标权重仍由 `core_trend` 矩阵格的 `metric_weight` 单独计算。

| Matrix Cell | 默认条数 | Allocation Weight |
|---|---:|---:|
| `problem_discovery:scenario_explore` | 3 | 7.5% |
| `problem_discovery:category_rec` | 6 | 15% |
| `solution_evaluation:scenario_explore` | 4 | 10% |
| `solution_evaluation:category_rec` | 5 | 12.5% |
| `solution_evaluation:deep_background` | 2 | 5% |
| `solution_evaluation:competitive_comp` | 6 | 15% |
| `purchase_decision:vendor_choice` | 4 | 10% |
| `purchase_decision:internal_justification` | 2 | 5% |
| `purchase_decision:purchase_risk` | 2 | 5% |
| `purchase_decision:commercial_terms` | 2 shadow | 5% |
| `purchase_decision:competitive_comp` | 4 | 10% |

合计 40 条。后续运营端调整总量时，仍按上述 allocation weight 自动折算每个矩阵格数量。
...
当前 40 条 query 的设计有很强业务直觉，但更像“兑吧业务理解版 Query 清单”，还不是一个严谨的 Answer Share 抽样框架。
