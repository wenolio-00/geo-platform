...
function buildFactorStrip(global) {
  return `<div class="factor-strip"><div class="factor-card"><div class="factor-v ${numberClass((global.natural_visibility ?? 0) * 100)}">${displayPercent(global.natural_visibility)}</div><div class="factor-l">自然可见度</div><div class="factor-sub">品牌出现在 AI 回答中的比例</div></div><div class="factor-card"><div class="factor-v">${displayValue(global.rank, { decimals: 2 })}</div><div class="factor-l">平均位次</div><div class="factor-sub">仅计已提及品牌且位置大于 0 的样本</div></div><div class="factor-card"><div class="factor-v ${numberClass((global.visibility ?? 0) * 100)}">${displayPercent(global.visibility)}</div><div class="factor-l">可见度</div><div class="factor-sub">自然可见度 ÷ 平均位次</div></div><div class="factor-card"><div class="factor-v ${numberClass((global.sentiment_score ?? 0) * 100)}">${displayPercent(global.sentiment_score)}</div><div class="factor-l">舆情指数</div><div class="factor-sub">正面=1.0 · 中立=0.5 · 负面=0.1 加权均值</div></div></div><div class="factor-formula">AI 推荐度 = 可见度 × 舆情指数 × 100 &nbsp;·&nbsp; 任意因子为 0 则得分为 0</div>`
}
...
function buildSources(data) {
  ...
  if (!sources.length && !gaps.length) return emptyState('暂无信源数据', '聚合数据未提供 sources 或 source_gap。')
  ...
  return `${sourceSection}${gapSection}`
}
