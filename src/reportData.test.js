...
describe('report data contract', () => {
  it('valid report_data fixture renders full report', () => {
    ...
    assert.match(html, /话题可见度 × 竞品对比/)
    assert.match(html, /全量竞品排名/)
    assert.match(html, /品牌调性分析/)
    assert.match(html, /较上期变化/)
    assert.match(html, /自然可见度/)
  ...
  it('omits hidden report sections from exported HTML', () => {
    const html = generateReportHtml(fixture, { hiddenSections: ['sources', 'sentiment', 'overview'] })
    assert.doesNotMatch(html, /信源引用情况/)
    assert.doesNotMatch(html, /品牌调性分析/)
    assert.doesNotMatch(html, /全局排名/)
    assert.match(html, /关键问题&amp;优化建议/)
    assert.match(html, /六平台健康度/)
  })

  it('can export an editable standalone report shell', () => {
    const plainHtml = generateReportHtml(fixture)
    const editableHtml = generateReportHtml(fixture, { editable: true })
    assert.doesNotMatch(plainHtml, /edit-toolbar/)
    assert.match(editableHtml, /edit-toolbar/)
    assert.match(editableHtml, /contenteditable/)
    assert.match(editableHtml, /导出干净 HTML/)
  })
